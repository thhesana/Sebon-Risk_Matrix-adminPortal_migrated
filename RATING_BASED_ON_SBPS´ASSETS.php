<?php
// rating_based_on_sbps_assets.php
include 'db.php';
include 'header.php';
...
// Selected fiscal year (default to first if not posted)
...
<div class="container" style="padding:20px;" id="printArea">
    <h2>Rating Based on SBPs' Assets - Linear Interpolation</h2>
...
// Aggregate assets per participant
$sql = "
    SELECT 
...
// Calculate rating
foreach ($allAssets as &$r) {
    $r['Percentage'] = ($totalSum > 0) ? ($r['TotalAssets'] / $totalSum) * 100.0 : 0.0;
    if ($r['TotalAssets'] > 0) {
        $r['Rating'] = ($maxAssets > $minAssets) ? calculateAssetRating($r['TotalAssets'], $minAssets, $maxAssets) : 1.0;
    } else {
        $r['Rating'] = 5.0; // Zero assets → worst rating
    }
    $r['RiskCategory'] = getRiskCategory($r['Rating']);
}
unset($r);
...
if (count($allAssets) === 0) {
    ...
    echo '<h3>LINEAR INTERPOLATION RESULTS</h3>';
    echo '<p><strong>Formula:</strong> Highest assets → Rating = 1.00 (Very low risk) | Lowest assets → Rating = 5.00 (Very high risk)</p>';
    echo '<p><strong>Min Assets:</strong> ' . number_format($minAssets, 2) . ' | <strong>Max Assets:</strong> ' . number_format($maxAssets, 2) . '</p>';

    echo '<table border="1" cellpadding="6" cellspacing="0" style="border-collapse:collapse; width:100%;">';
    echo '<thead style="background:#2c3e50; color:#fff;"><tr>
            <th>Position</th>
            <th style="text-align:left;">Name of Securities Business Persons</th>
    ...
    foreach ($allAssets as $row) {
        ...
        echo '<td style="text-align:center;">' . number_format($row['Percentage'], 1) . '%</td>';
        echo '<td style="text-align:center;">' . ($row['TotalAssets']>0 ? number_format($row['TotalAssets'],0) : '-') . '</td>';
        echo '<td style="text-align:center; font-weight:700; color:' . $color . ';">' . number_format($row['Rating'], 2) . '</td>';
        echo '<td style="text-align:center; font-weight:700; color:' . $color . ';">' . htmlspecialchars($row['RiskCategory']) . '</td>';
        echo '</tr>';
        $pos++;
    }

    echo '<tr style="background:#f8f9fa; font-weight:700;"><td colspan="2" style="text-align:right;">Sum Total</td>';
    echo '<td style="text-align:right;">' . number_format($totalSum,2) . '</td>';
    ...
    echo '<h4>Risk Scale Reference</h4>';
    echo '<table border="1" cellpadding="6" cellspacing="0" style="border-collapse:collapse; width:40%;">';
    echo '<thead style="background:#34495e; color:#fff;"><tr><th>Scale</th><th>From</th><th>To</th></tr></thead><tbody>';
    echo '<tr><td style="color:#27ae60; font-weight:700;">Very low</td><td>1.00</td><td>1.20</td></tr>';
    echo '<tr><td style="color:#2ecc71; font-weight:700;">Low</td><td>1.21</td><td>1.80</td></tr>';
    echo '<tr><td style="color:#f39c12; font-weight:700;">Medium</td><td>1.81</td><td>3.40</td></tr>';
    echo '<tr><td style="color:#e67e22; font-weight:700;">High</td><td>3.41</td><td>4.20</td></tr>';
    echo '<tr><td style="color:#e74c3c; font-weight:700;">Very high</td><td>4.21</td><td>5.00</td></tr>';
